...
void print_reply(int rc)
{
    ...
    {
        case 220:
            printf("220 Welcom, server ready.\n");
            break;
        case 221:
            printf("221 Goodbye!\n");
            break;
...
int ftclient_get(int data_sock, int sock_control, char* arg)
{
    char data[MAXSIZE];
    int size;
    FILE* fd = fopen(arg, "w"); //创建并打开arg的文件
...
/**
 * 输入含有命令（code）和参数（arg）的command（cmd）结构
 * 链接code+arg，并放进一个字符串，然后发送给服务器
 */
...
int main(int argc, char* argv[])
{
    ...
    char *port = argv[2];   //所要连接到副取其程序端口号
