...
void ftserve_retr(int sock_control, int sock_data, char* filename)
{
    ...
    if(!fd)
        send_response(sock_control, 550);   //发送错误码（550，Requestd action not taken)
    ...
    else
    {
        ...
        send_response(sock_control, 226);   //发送消息： 226：closing conn， file stansfer successful
...
int ftserve_list(int sock_data, int sock_control)
{
    ...
    int rs = system("ls -l | tail -n+2 > tmp.txt"); //利用系统调用函数 system 执行命令，并重定向到tmp.txt 文件
    if(rs < 0)
    {
        exit(1);
    }
    ...
    send_response(sock_control, 226);   //发送应答码226（关闭数据连接，请求的文件操作成功）
